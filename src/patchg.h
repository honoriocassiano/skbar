...
template <class MeshType, class PatchParam> class PatchG {
public:
  ...
  void generate_topology(const PatchParam &param) {
    ...
    std::string param_str =
        patchgen::get_param_str(num_sides, param.pattern_id, param);
    // if (!param_str.empty())
    ...
    /*for (auto v : patch.vertices()) {
        ...
        if (param.permutation.is_flipped()) corner_index = (corner_index + 1) %
    num_sides;      // Be careful!
    }
  ...
  void determine_geometry(const Eigen::VectorXi &l) {
    ...
    for (typename MeshType::FaceIterator fi = mesh.face.begin();
         fi != mesh.face.end(); fi++) {
    ...
    if (num_sides > 2) {
      ...
      for (int i = 0; i < frontiers.size(); i++) {
        ...
        if (frontiers[i]->IsS()) {
          ...
          cout << "corner index " << vcg::tri::Index(mesh, frontiers[i])
               << endl;
          t++;
        }
      }
      ...
      for (int i = 0; i < num_sides; i++) {
        ...
        for (int j = 1; j < sideIndexes.size() - 1; j++) {
          ...
          // cout<<"point "<<mesh.vert[sideIndexes[j]].P().X()<<"
          // "<<mesh.vert[sideIndexes[j]].P().Y()<<"
          // "<<mesh.vert[sideIndexes[j]].P().Z()<<endl;
          // cout<<"point "<<mesh.vert[sideIndexes[j]].P().X()<<"
          // "<<mesh.vert[sideIndexes[j]].P().Y()<<"
          // "<<mesh.vert[sideIndexes[j]].P().Z()<<endl;
  ...
  void
  determine_geometry(const vector<vector<typename MeshType::CoordType>> &l) {
    ...
    auto side_indexL =
        vcg::tri::Allocator<MeshType>::template GetPerVertexAttribute<int>(
            mesh, std::string("LeftSide"));
    auto side_indexR =
        vcg::tri::Allocator<MeshType>::template GetPerVertexAttribute<int>(
            mesh, std::string("RightSide"));
    ...
    int Mask = 0;
    string
    aux="../exported/patchmesh2D"+std::to_string(numberSides)+"-"+std::to_string(mesh.vert.size())+".obj";
    ...
    if (sideIndexes[correspondence[0].second].back() !=
        sideIndexes[correspondence[1].second][0])
    ...
    for (int i = 0; i < numberSides; i++) {
      ...
      for(const int &index : positions)
          cout<<index<<" ";
      cout<<endl;
      cout<<" sidel 0"<<side_indexL[positions[0]]<<", sideR
      0"<<side_indexR[positions[0]];
      cout<<" sidel end"<<side_indexL[positions.back()]<<", sideR
      end"<<side_indexR[positions.back()]<<endl;*/
      /*cout<<"left 0 "<<side_indexL[positions[0]]<<endl;
      cout<<"right 0 "<<side_indexR[positions[0]]<<endl;
      cout<<"left final "<<side_indexL[positions.back()]<<endl;
      cout<<"right final "<<side_indexR[positions.back()]<<endl;*/
      // assert(side_indexL[positions[0]]==i);
      // assert(side_indexR[positions.back()]==i);
      // cout<<"---Populate side "<<i<<endl;
      // cout<<"number points "<<positions.size()<<endl;

      for (int j = 0; j < positions.size(); j++) {

        // cout<<"mesh index "<<positions[j]<<", index "<<j<<" associated [
        // "<<l[i][j].X()<<" "<<l[i][j].Y()<<endl;
    ...
    // Correcting points that remain outside the boundary of the patch (due to
    // the large difference between the number of divisions on one side of the
    // patch and the others)
    ...
    do {
      ...
      if (!utility::pointIsInsidePoly(boundary, copy.VFlip()->P())) {
        ...
        CoordType reflexion1, reflexion2; //  actually, we must be check the
                                          //  valence equal 2, but for while ...
        utility::computeReflexion(reflexion1, P, copy.V()->P(),
                                  copy.VFlip()->P());
        utility::computeReflexion(reflexion1, P, copy.V()->P(),
                                  copy.VFlip()->P());
      }
  ...
  void create_and_process(
      const vector<vector<typename MeshType::CoordType>> &borderpatch) {
  ...
  vector<int> getSideK(int side) {
      ...
      for (int k = 0; k < fi->VN(); k++) {
        ...
        if (fi->V(k)->IsS()) {
          ...
          if (side_indexR[fi->V(k)] == side) {
            ...
            start.Set(&*fi, k, fi->V(k));
            int count = 0;
            while (side_indexL[start.VFlip()] != side && count < 100) {
            ...
            if (count > 99)
              cout << "PROBLEM WITH THE getSidek" << endl;
            start.FlipV();
    ...
    do {
      ...
      do {
        start.NextE();
      } while (side_indexL[start.VFlip()] != side &&
               side_indexR[start.VFlip()] != side);
