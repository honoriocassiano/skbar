...
protected:
    ...
    std::string buttons_to_name(choice c) const
    {
        switch (c)
        {
    ...
    std::string get_icon_name(icon i) const
    {
        switch (i)
        {
...
public:
    notify(std::string const &title,
           ...
           icon i = icon::info)
    {
        if (i == icon::question) // Not supported by notifications
            i = icon::info;
        ...
        else if (is_zenity())
        {
            command += " --notification"
                       " --window-icon " + get_icon_name(i) +
                       " --text " + shell_quote(title + "\n" + message);
        }
        else if (is_kdialog())
        {
            command += " --icon " + get_icon_name(i) +
                       " --title " + shell_quote(title) +
...
public:
            ...
            icon i = icon::info)
    {
        ...
        switch (i)
        {
            case i::warning: style |= MB_ICONWARNING; break;
            case i::error: style |= MB_ICONERROR; break;
            case i::question: style |= MB_ICONQUESTION; break;
            /* case icon::info: */ default: style |= MB_ICONINFORMATION; break;
        }

        switch (c)
        {
            case c::ok_cancel: style |= MB_OKCANCEL; break;
            case c::yes_no: style |= MB_YESNO; break;
            case c::yes_no_cancel: style |= MB_YESNOCANCEL; break;
            case c::retry_cancel: style |= MB_RETRYCANCEL; break;
            case c::abort_retry_ignore: style |= MB_ABORTRETRYIGNORE; break;
        ...
        else if (is_zenity())
        {
            ...
            command += " --title " + shell_quote(title)
                     ...
                     + " --icon-name=dialog-" + get_icon_name(i);
        }
        else if (is_kdialog())
        {
            if (c == choice::ok)
            ...
            {
                command += " --";
                if (i == icon::warning || i == icon::error)
                    command += "warning";
                command += "yesno";
                if (c == choice::yes_no_cancel)
                {
                    m_mappings[256] = button::no;
                    command += "cancel";
                }
